...
def crear_data_modelo(datos, var_cont, var_categ, var_interac=[]):
    ...
    if len(var_categ) > 0:
        datos_v = pd.get_dummies(datos_v, columns=var_categ, drop_first=True).astype(float)  #evitar que aparezcan booleanos en el df
    ...
    if len(var_interac) > 0:
        ...
        datos_interac = datos_interac.astype(float)  #evitar que aparezcan booleanos en el df
