...
def plot_corr_cos(n_components, correlaciones_datos_con_cp):
    """
    Genera gráficos que representan vectores por cada variable, usando como ejes las componentes principales.
    La orientación y longitud del vector representa la correlación entre cada variable y dos componentes.
    ...
    Args:
        ...
        correlaciones_datos_con_cp (DataFrame): DataFrame con la matriz de correlaciones entre variables y componentes
    """
    # Definir mapa de color
    cmap = plt.get_cmap('coolwarm')

    for i in range(n_components):
        for j in range(i + 1, n_components):
            # Calcular suma de cosenos al cuadrado
            ...
            # Dibujar vectores para cada variable
            for k, var_name in enumerate(correlaciones_datos_con_cp.index):
                ...
                # Color basado en suma de cosenos
                color = cmap(sum_cos2[k])

                # Dibujar vector y etiqueta
...
def plot_pca_scatter_with_categories(datos_componentes_sup_var, componentes_principales_sup, n_components, var_categ):
    ...
    if isinstance(componentes_principales_sup, pd.DataFrame):
        componentes_principales_sup = componentes_principales_sup.values
...
def plot_pca_scatter_with_categories2(datos_componentes_sup_var, componentes_principales_sup, n_components, var_categ):
    """
    Genera gráficos de dispersión de observaciones en pares de componentes principales seleccionados con categorías.

    Args:
        datos_componentes_sup_var (pd.DataFrame): DataFrame que contiene las categorías.
        componentes_principales_sup (pd.DataFrame/np.ndarray): Matriz de componentes principales.
        n_components (int): Número de componentes principales seleccionadas.
        var_categ (str): Nombre de la variable introducida
    """
    # Convertir a numpy array si es DataFrame
    ...
    for i in range(n_components):
        for j in range(i + 1, n_components):
            ...
            for idx, categoria in enumerate(categorias):
                mask = datos_componentes_sup_var[var_categ] == categoria
                # Puntos de cada categoría
                ax.scatter(
                    componentes_principales_sup[mask, i],
                    componentes_principales_sup[mask, j],
                    c=[colores[idx]],
                    label=categoria,
                    alpha=0.6
                )
                ...
                observaciones_categoria = componentes_principales_sup[mask]
                centroide = np.mean(observaciones_categoria, axis=0)
                ax.scatter(
                    centroide[i],
                    centroide[j],
                    ...
                    edgecolor='black',
                    linewidth=1.5,
                    label=f'Centroide {categoria}'
                )

            # Añadir etiquetas a las observaciones
            for k, label in enumerate(datos_componentes_sup_var.index):
                ax.annotate(
                    label,
                    (componentes_principales_sup[k, i], componentes_principales_sup[k, j]),
                    xytext=(5, 5),
            ...
            ax.set_xlabel(f'Componente Principal {i + 1}')
            ax.set_ylabel(f'Componente Principal {j + 1}')
            ax.set_title('Distribución de Observaciones por Especies en PCA')

            # Leyenda
            ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')

            # Grid y ajuste de layout
